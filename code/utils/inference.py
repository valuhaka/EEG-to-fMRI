## Luka Vähäsarja, August 2024
## Based on Kovalev et al. (https://github.com/kovalalvi/beira)
...
def corr_results_plot(labels, corrs):
    ...
    labels_roi = labels
    df = pd.DataFrame({"roi_name": labels_roi, "corrs": corrs})
    ...
    correctness = False
    correctness = (
        left_df["roi_name"].str.replace("Left", "")
        == right_df["roi_name"].str.replace("Right", "")
    ).all()
    ...
    plt.xticks(x, left_df["roi_name"].str.replace("Left", ""), rotation=90)
    plt.ylim(-0.2, 1)
...
def make_visualization2(y_prediction, y_test, labels):
    """
    Calculate correlation metrics for each roi between y_pred and y_test.
    Visaulize via n_roi plots
    ...
    fig, ax = plt.subplots(math.ceil(n_roi / 2), 2, figsize=(8, 8), dpi=120, sharex=True, sharey=True)
    
    # order = [0, 4, 1, 5, 2, 6, 3, 7]

    n_lab = len(labels)
    order = np.floor(np.arange(0, n_lab / 2, 0.5))  # make [0, 0, 1, 1, ...]
    for ind in range(n_lab):
        if ind % 2:
            order[ind] += n_lab / 2  # add n_lab/2 to every other number
    ...
    for roi in range(len(labels)):
        ...
        axi.plot(time, y_test_roi, label="true", color="blue",alpha=0.5)  # True signal in orange
        axi.plot(time, y_hat, label="prediction", color="orange")
        # axi.set_xlim(0, 60)
...
def make_visualization(y_prediction, y_test, labels):
    ...
    # Change the figure size to be wider and create only one subplot
    ...
    time = np.arange(len(y_hat)) / 100
    ax.plot(time, y_test_roi, label="true", color="orange")  # True signal in orange
    ax.plot(time, y_hat, label="prediction", color="blue")

    ax.set_title("{} r = {:.2f}".format(labels[roi], corr_tmp))
...
def make_inference_seq_2_seq(model, dataset, device="cpu"):
    """
    Make inference for model many2many. So we take sequence of input data and get sequence of output.
...
def model_inference_function(model, dataset, labels, device="cuda", to_many=False):
    """
    ...
    return fig, fig_bars, corrs, y_hats, y_test


def find_best_shift(fmri, y_test):
  """Finds the best shift of y_test within fmri."""

  best_shift = 0
  min_difference = float('inf')

  for shift in range(len(fmri) - len(y_test) + 1):
    fmri_section = fmri[shift : shift + len(y_test)]
    difference = np.sum(np.abs(fmri_section - y_test))
...
def calculate_metric(y_hat, y_true, func_metric_1, func_metric_2, crop_lenght=None):
    ...
    for i in range(n_crops):
        ...
        for n_feature in range(y_hat_crop.shape[-1]):
            metric_1 = func_metric_1(
                y_hat_crop[:, n_feature], y_true_crop[:, n_feature]
            )
            metric_2 = func_metric_2(
                y_hat_crop[:, n_feature], y_true_crop[:, n_feature]
            )
