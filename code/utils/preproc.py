...
def downsample_ts(data, factor=1, axis=-1):
    if factor == 1:
        return data
    data_ds = mne.filter.resample(data, up=1, down=factor, axis=-1)
    return data_ds
...
def compute_wavelet(data, sf, freqs):
    ...
    """
    n_chan = data.shape[0]
    wavelet_power = mne.time_frequency.tfr_array_morlet(
        data.reshape((1, n_chan, -1)), sfreq=sf, freqs=freqs, output="power"
    )
...
def low_level_preproc_eeg(eeg, eeg_fq, verbose):
    """
    Apply filtering and common rereference.
    eeg should be size ( n_electrodes, time)
    fps - Hz
    """
    eeg_filter = filter_powerline_noise(eeg, sf=eeg_fq, verbose=verbose)
    eeg_filter = mne.filter.filter_data(
        eeg_filter, sfreq=eeg_fq, l_freq=1, h_freq=100, verbose=verbose
    )
...
def mid_level_preproc_eeg(eeg, eeg_fq, verbose):
    """
    Apply filtering and normalise by run
    eeg should be size ( n_electrodes, time)
    fps - Hz
    """
    eeg_filter = filter_powerline_noise(eeg, sf=eeg_fq, verbose=verbose)
    ...
    run_len = eeg_filter.shape[1] // 4

    eeg_list = []

    for run in range(4):
        run_data = eeg[1 : eeg.shape[0], run_len * run : run_len * (run + 1)]
        run_std = np.std(run_data, axis=1)
        run_data = run_data.T / run_std
        eeg_list.append(pd.DataFrame(run_data))

    eeg_filter = pd.concat(eeg_list, axis=0)
    eeg_filter.insert(0, eeg[0])  # insert times
    eeg_filter = eeg_filter.to_numpy()

    return eeg_filter


def preproc_dataset(
    dataset, fps, freqs, crop_size=None, inp_means_stds=None, out_means_stds=None
):
    """
    ...
    eeg_filter = mne.filter.filter_data(
        eeg_filter, sfreq=fps, l_freq=1, h_freq=100, verbose=False
    )
    ...
    print(
        f"Size of eeg features {eeg_wavelet_features.shape} | fmri {fmri_arrays.shape}"
    )
